...
def remove_empty(original_dict: dict, inplace=False) -> dict:
    ...
    for key in sorted(original_dict, reverse=True):
        if inplace is True:
            count = 1
            if original_dict[key] != [] and original_dict[key] != () and original_dict[key] != '' and original_dict[key] != None:
                second_dict[key] = original_dict[key]
        else:
            if original_dict[key] == [] or original_dict[key] == () or original_dict[key] == '' or original_dict[key] == {} or original_dict[key] == None:
